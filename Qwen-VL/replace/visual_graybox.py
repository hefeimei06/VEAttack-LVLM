...
def pgd_veattack(
    ...
    for i in range(iterations):
        ...
        with torch.enable_grad():
            tokens = forward(data_clean + perturbation, visual_feature=True)
            loss = loss_fn(tokens)
            if verbose:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, visual_feature: bool = False):
        x = x.to(
    ...
    def encode(self, image_paths: List[str]):
        ...
        eps = 8 / 255
        with torch.no_grad():
            tokens_orig = self(images, visual_feature=True)

        loss_inner_wrapper = ComputeLossWrapper(tokens_orig, 'mean')
        norm = 'linf'
        iterations_adv = 100
        stepsize_adv = 4 / 255
        images = pgd_veattack(
